...
const PERMISSIONS: Permission[] = [
  // Leads
  { key: 'org:leads_read:leads_read', name: 'Leads: Leitura', description: 'Visualizar leads e informações de prospects' },
  { key: 'org:leads_write:leads_write', name: 'Leads: Escrita', description: 'Criar, editar e gerenciar leads' },

  // Conversations
  { key: 'org:conversations_read:conversations_read', name: 'Conversas: Leitura', description: 'Visualizar conversas e mensagens' },
  { key: 'org:conversations_write:conversations_write', name: 'Conversas: Escrita', description: 'Enviar mensagens e gerenciar conversas' },

  // Students
  { key: 'org:students_read:students_read', name: 'Alunos: Leitura', description: 'Visualizar informações de alunos' },
  { key: 'org:students_write:students_write', name: 'Alunos: Escrita', description: 'Criar, editar e gerenciar alunos' },

  // Tickets
  { key: 'org:tickets_read:tickets_read', name: 'Tickets: Leitura', description: 'Visualizar tickets de suporte' },
  { key: 'org:tickets_write:tickets_write', name: 'Tickets: Escrita', description: 'Criar e gerenciar tickets de suporte' },

  // Reports
  { key: 'org:reports_read:reports_read', name: 'Relatórios: Leitura', description: 'Visualizar relatórios e métricas' },
];
...
async function createRole(role: Role) {
  try {
    ...
    if (error.message.includes('already exists') || error.message.includes('409') || error.message.includes('404') || error.message.includes('422') || error.message.includes('taken')) {
...
async function updateRolePermissions(role: Role) {
  try {
    ...
    if (existingRole) {
      ...
      console.log(`✅ Permissões atualizadas para role: ${role.key} (${permissionIds.length} permissões)`);
      return result;
    }
  } catch (error: any) {
