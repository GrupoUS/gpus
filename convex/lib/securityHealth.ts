...
async function checkEncryptionSecurity(ctx: QueryCtx): Promise<{
	...
	const studentsWithCpf = await ctx.db
		.query('students')
		.filter((q) =>
			q.and(q.neq(q.field('cpf'), undefined), q.eq(q.field('encryptedCPF'), undefined)),
		)
		.take(10);
...
async function checkAuthSecurity(ctx: QueryCtx): Promise<{
	...
	const usersWithoutRoles = await ctx.db
		.query('users')
		.filter((q) => q.or(q.eq(q.field('role'), undefined), q.eq(q.field('role'), '')))
		.take(10);
	...
	const inactiveUsers = await ctx.db
		.query('users')
		.filter((q) => q.and(q.eq(q.field('isActive'), false), q.neq(q.field('clerkId'), undefined)))
		.take(10);
...
async function checkDataProtection(ctx: QueryCtx): Promise<{
	...
	const studentsWithoutConsent = await ctx.db.query('lgpdConsent').collect();

	const uniqueStudentsWithConsent = new Set(
		studentsWithoutConsent.map((c) => c.studentId.toString()),
	);
...
async function checkMonitoringSecurity(ctx: QueryCtx): Promise<{
	...
	if (recentAuditLogs.length < totalStudents.length * 0.1) {
	...
	const auditLogTypes = new Set(
		recentAuditLogs.map((log: { actionType: string }) => log.actionType),
	);
	const requiredTypes = [
		'data_creation',
		'data_modification',
