...
/**
 * List Clerk Users
 * POST /clerk/users/list
 */
http.route({
  path: "/clerk/users/list",
  method: "POST",
  handler: httpAction(async (_ctx, request) => {
    ...
    try {
      const { limit = 10, offset = 0, query } = await request.json();
      ...
      return new Response(JSON.stringify(users), {
       ...
       console.error("Clerk List Users Error:", error);
       return new Response("Internal Server Error", { status: 500 });
    }
  }),
});
...
/**
 * Invite Clerk User
 * POST /clerk/users/invite
 */
http.route({
  path: "/clerk/users/invite",
  method: "POST",
  handler: httpAction(async (_ctx, request) => {
    const clientIP = request.headers.get("x-forwarded-for")?.split(",")[0] || "unknown";
    if (!webhookRateLimiter.isAllowed(clientIP)) return new Response("Too Many Requests", { status: 429 });

    try {
      const { email, role, redirectUrl } = await request.json();
...
/**
 * Update User Role
 * POST /clerk/users/update-role
 */
http.route({
  path: "/clerk/users/update-role",
  method: "POST",
  handler: httpAction(async (_ctx, request) => {
     const clientIP = request.headers.get("x-forwarded-for")?.split(",")[0] || "unknown";
     if (!webhookRateLimiter.isAllowed(clientIP)) return new Response("Too Many Requests", { status: 429 });
     ...
     try {
       ...
       return new Response(JSON.stringify({ success: true }), { status: 200 });
        ...
        console.error("Clerk Update Role Error:", error);
        return new Response(JSON.stringify({ error: error.message }), { status: 500 });
     }
  }),
});
 ...
 * POST /clerk/users/remove
 */
http.route({
  path: "/clerk/users/remove",
  method: "POST",
  handler: httpAction(async (_ctx, request) => {
    const clientIP = request.headers.get("x-forwarded-for")?.split(",")[0] || "unknown";
     if (!webhookRateLimiter.isAllowed(clientIP)) return new Response("Too Many Requests", { status: 429 });
     ...
     try {
       ...
       return new Response(JSON.stringify({ success: true }), { status: 200 });
        ...
        console.error("Clerk Remove User Error:", error);
        return new Response(JSON.stringify({ error: error.message }), { status: 500 });
     }
  }),
});
...
/**
 * Get User Details
 * GET /clerk/users/:userId
 */
http.route({
  pathPrefix: "/clerk/users/",
  method: "GET",
  handler: httpAction(async (_ctx, request) => {
    const url = new URL(request.url);
