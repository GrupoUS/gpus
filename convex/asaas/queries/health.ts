...
export const getWebhookHealth = internalQuery({
  ...
  handler: async (ctx) => {
    ...
    const total =
      pending.length + processing.length + done.length + failed.length;
...
export const getEventTypeDistribution = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const paymentEvents = recentWebhooks.filter((w) =>
      w.event.startsWith("PAYMENT_"),
    ).length;
    ...
    const customerEvents = recentWebhooks.filter((w) =>
      w.event.startsWith("CUSTOMER_"),
    ).length;
