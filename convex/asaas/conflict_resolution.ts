  ...
  handler: async (ctx, args) => {
    ...
    if (args.action === "link" && conflict.asaasCustomerId) {
      ...
      await ctx.db.patch(args.conflictId, {
        ...
        resolvedAt: now,
        resolutionNote:
          args.resolutionNote || "Linked student to Asaas customer",
        updatedAt: now,
      });
...
export const resolveConflictManually = mutation({
  ...
  handler: async (ctx, args): Promise<{ success: boolean; action: string }> => {
    ...
    return await ctx.runMutation(
      internal.asaas.conflict_resolution.resolveCustomerConflict,
      {
      ...
      .filter((q) =>
        q.and(
          ...
          args.asaasCustomerId
            ? q.eq(q.field("asaasCustomerId"), args.asaasCustomerId)
            : true,
        ),
      )
      .first();
