...
export const sendTestWebhook = action({
  ...
  handler: async (ctx, args): Promise<any> => {
    ...
    if (args.customPayload) {
      ...
      if (!template) {
        throw new Error(`Unknown event type: ${args.eventType}. Available: ${Object.keys(TEST_PAYLOADS).join(", ")}`);
      }
      ...
      if (typeof template === "function") {
        ...
        switch (args.eventType) {
          ...
          default:
            ...
            payload = (template as (id: string) => Record<string, unknown>)(`test_dup_${Date.now()}`);
        }
      } else {
        payload = template as Record<string, unknown>;
      }
    ...
    try {
      ...
      const result = await ctx.runAction(
        internal.asaas.webhooks.processWebhookIdempotent,
        {
