    ...
    try {
      if (args.testScenario === "happy_path") {
        ...
        try {
            ...
            const badClient = new AsaasClient({ apiKey: "invalid_key", baseUrl: process.env.ASAAS_BASE_URL });
        ...
        results.details.note = "Real network timeout simulation skipped in live env to protect circuit breaker";
      }
...
export const loadTestSync = action({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(`[LoadTest] Starting sync of ${customerCount} customers (batch size: ${batchSize})`);
    ...
    for (let i = 0; i < testCustomers.length; i += batchSize) {
      ...
      console.log(`[LoadTest] Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(customerCount / batchSize)}`);
    ...
    console.log(`[LoadTest] Completed: ${results.successful}/${customerCount} successful in ${results.totalTimeMs}ms`);
...
function generateTestCPF(seed: number): string {
  ...
  const seededRnd = (n: number, offset: number) => Math.round(((seed + offset) * 9301 + 49297) % 233280 / 233280 * n);
