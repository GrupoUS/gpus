...
export async function exportStudentWorker(
  ...
  try {
    ...
    const asaasCustomer: AsaasCustomerResponse =
      await asaasClient.createCustomer(customerPayload);
    ...
    console.error(
      `[ExportStudentWorker] Failed to export student ${student._id}:`,
      error.message,
    );
...
export async function exportPaymentWorker(
  ...
  try {
    ...
    const asaasPayment: AsaasPaymentResponse =
      await asaasClient.createPayment(paymentPayload);
    ...
    console.error(
      `[ExportPaymentWorker] Failed to export payment ${payment._id}:`,
      error.message,
    );
