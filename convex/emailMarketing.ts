...
export const syncLeadAsContactInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!lead.email) {
      console.log(
        `[EmailMarketing] Lead ${args.leadId} has no email, skipping sync`,
      );
      return null;
    }
    ...
    if (existing) {
      console.log(
        `[EmailMarketing] Lead ${args.leadId} already synced as contact ${existing._id}`,
      );
      return existing._id;
    }
    ...
    console.log(
      `[EmailMarketing] Auto-synced lead ${args.leadId} as contact ${contactId}`,
    );
    return contactId;
  },
});
...
export const syncStudentAsContactInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!student) {
      console.log(
        `[EmailMarketing] Student ${args.studentId} not found for sync`,
      );
      return null;
    }
    if (!student.email) {
      console.log(
        `[EmailMarketing] Student ${args.studentId} has no email, skipping sync`,
      );
      return null;
    }
    ...
    console.log(
      `[EmailMarketing] Auto-synced student ${args.studentId} as contact ${contactId}`,
    );
    return contactId;
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(internalEmailMarketing.updateCampaignStats, {
      ...
      stats: {
        ...
        bounced:
          (globalStats?.hardBounces || 0) + (globalStats?.softBounces || 0),
    ...
    for (const contactData of args.contacts) {
      ...
      if (!existing) {
        ...
        await ctx.db.insert("emailContacts", {
          ...
          leadId:
            contactData.sourceType === "lead"
              ? (contactData.sourceId as any)
              : undefined,
          studentId:
...
export const createListFromSegment = action({
  ...
  handler: async (ctx, args) => {
    ...
    const contacts = (await ctx.runQuery(
      internalEmailMarketing.getSegmentDataInternal,
      {
        sourceType: args.sourceType,
        filters: args.filters,
      },
    ...
    const listId = await ctx.runMutation(internalEmailMarketing.createList, {
      name: args.name,
      description:
        args.description || `Criada a partir de segmento ${args.sourceType}`,
    });
