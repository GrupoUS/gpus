  ...
  handler: async (ctx, args) => {
    ...
    if (args.search) {
      ...
      return students.filter(
        (s) => s.name && s.name.toLowerCase().includes(searchLower),
      );
    }
  ...
  handler: async (ctx, args) => {
    ...
    try {
      if (student.encryptedCPF)
        student.cpf = await decryptCPF(student.encryptedCPF);
    } catch (e) {
    ...
    try {
      if (student.encryptedEmail)
        student.email = await decrypt(student.encryptedEmail);
    } catch (e) {
      console.warn(`Failed to decrypt email for student ${args.id}:`, e);
      student.email =
        student.email || "[Email criptografado - falha na decriptação]";
    }

    try {
      if (student.encryptedPhone)
        student.phone = await decrypt(student.encryptedPhone);
    } catch (e) {
      console.warn(`Failed to decrypt phone for student ${args.id}:`, e);
      student.phone =
        student.phone || "[Telefone criptografado - falha na decriptação]";
    }
...
export const diagnoseOrganizationId = query({
  ...
  handler: async (ctx) => {
    ...
    const studentsMatchingOrg = allStudents.filter(
...
export const getChurnAlerts = query({
  ...
  handler: async (ctx) => {
    try {
      ...
      students.sort(
        (a, b) => (a.lastEngagementAt || 0) - (b.lastEngagementAt || 0),
      );
      ...
      for (const student of students) {
        ...
        if (
          student.lastEngagementAt &&
          student.lastEngagementAt < thirtyDaysAgo
        ) {
          alerts.push({
            _id: student._id,
            studentName: student.name,
  ...
  handler: async (ctx, args) => {
    ...
    if (args.email) {
      try {
        await ctx.scheduler.runAfter(
          0,
          internal.emailMarketing.syncStudentAsContactInternal,
          {
            studentId,
            organizationId,
          },
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.patch.email)
      updates.encryptedEmail = await encrypt(args.patch.email);
    if (args.patch.phone)
      updates.encryptedPhone = await encrypt(args.patch.phone);
