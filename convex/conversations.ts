...
export const list = query({
	args: {
		...
		department: v.optional(v.union(v.literal('vendas'), v.literal('cs'), v.literal('suporte'))),
	...
	handler: async (ctx, args) => {
		...
		if (args.department) {
			...
			conversations = await ctx.db.query('conversations').order('desc').take(50);
		}
		...
		if (args.search) {
			...
			filtered = filtered.filter((c) => c.contactName.toLowerCase().includes(searchLower));
		}
