...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Component requires multiple state checks and UI logic
export function AdminExportDialog() {
	...
	const paymentsToExport = useQuery(api.asaas.getPendingPayments, {});
	...
	const totalToExport =
		exportType === 'students'
			...
			: exportType === 'payments'
				? paymentsToExport?.length || 0
				: (studentsToExport?.length || 0) + (paymentsToExport?.length || 0);
				...
				<div className="space-y-6">
					...
					<div className="space-y-3">
						...
						<div className="text-sm text-muted-foreground">
							...
							{exportType === 'payments' && `${paymentsToExport?.length || 0} pagamentos pendentes`}
							{exportType === 'all' &&
								`${(studentsToExport?.length || 0) + (paymentsToExport?.length || 0)} registros totais`}
						</div>
					</div>
