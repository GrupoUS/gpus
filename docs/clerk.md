...
2. Install the Clerk React SDK with `bun add @clerk/clerk-react@latest`.
...
```bash
bun create vite@latest clerk-react -- --template react-ts
cd clerk-react
bun install
bun dev
```
...
```bash
bun add @clerk/clerk-react@latest
```
